...
def plot_model_fits(t_binned, w_binned, sig_w_binned, idata_single, idata_binary, batch_name, i):
    ...
    plt.tight_layout()
    if batch_name is not None and i is not None:
        plt.savefig('%s/plots/model_comparison_fits_%s.png'%(batch_name, i), dpi=300, bbox_inches='tight')
        plt.close()
    return fig
...
def generate_corner_plot(idata, model='binary', fig=None, use_campbell=True, true_vals=None, initvals=None):
        ...
        if model == 'binary': 
            if use_campbell:
                plot_list = ['delta_dec', 'delta_ra', 'pm_ra', 'pm_dec', 'log_parallax', 'mp', 'log_a0', 'log_p', 'ecc', 'phi', 'cosi', 'Omega', 'omega']
                labels = [
                            r"$\Delta\alpha$ [mas]", r"$\Delta\delta$ [mas]", r"$\mu_{\alpha}$ [mas/yr]", r"$\mu_{\delta}$ [mas/yr]",
                            r"$\log{\varpi}$ [as]", r"$M_P$ [$M_J$]", r"$\log{a_0}$ [as]", r"$\log{P}$ [days]", r"$e$",
                            r"$\phi$", r"$\cos{i}$", r"$\Omega$", r"$\omega$"
                        ]
            else:
                plot_list = ['delta_dec', 'delta_ra', 'pm_ra', 'pm_dec', 'log_parallax', 'mp', 'log_a0', 'log_p', 'ecc', 'phi', 'A_in_mas', 'B_in_mas', 'F_in_mas', 'G_in_mas', 'cosi', 'Omega', 'omega']
                labels = [
                            r"$\Delta\alpha$ [mas]", r"$\Delta\delta$ [mas]", r"$\mu_{\alpha}$ [mas/yr]", r"$\mu_{\delta}$ [mas/yr]",
                            r"$\log{\varpi}$ [as]", r"$M_P$ [$M_J$]", r"$\log{a_0}$ [as]", r"$\log{P}$ [days]", r"$e$", r"$\phi$",
                            r"A [mas]", r"B [mas]", r"F [mas]", r"G [mas]", r"$\cos{i}$", r"$\Omega$", r"$\omega$"
                        ]
                
        elif model == 'single':
            plot_list = ['delta_dec', 'delta_ra', 'pm_ra', 'pm_dec', 'log_parallax']
            labels = [r"$\Delta\alpha$ [mas]", r"$\Delta\delta$ [mas]", r"$\mu_{\alpha}$ [mas/yr]", r"$\mu_{\delta}$ [mas/yr]", r"$\log{\varpi}$ [as]"]
