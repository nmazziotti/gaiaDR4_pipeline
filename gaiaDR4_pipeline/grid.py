...
def grid_search_torch(t_ast_yr, psi, plx_factor, ast_obs, ast_err, P_range=[10, 1e4], use_logspace=True, device="cuda", gpu_chunk_size=100000, N_logP = 1800, N_ecc=60,  N_phi=60):
    dtype = torch.float64  
...
def run_grid_search(cwd, source_id, t_binned, psi_binned, pf_binned, w_binned, sig_w_binned, device="cuda", **kwargs):
    ...
    start = time.time()
    log_P_samples, ecc_samples, phi_samples, mu_samples, chi2_samples, weights, log_P_grid, ecc_grid, phi_grid = grid_search_torch(
